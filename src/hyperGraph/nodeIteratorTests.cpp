#include "/Users/Eddie/hyperGraphEM/src/hyperGraph/hyperGraph.h"
#include "/Users/Eddie/hyperGraphEM/src/pedigreeParse/pedigreeToPedigree.h"
#include "/Users/Eddie/hyperGraphEM/src/messages/errorMessages.h"
#include "/Users/Eddie/hyperGraphEM/src/helper/nestedLoop.h"
#include "/Users/Eddie/hyperGraphEM/src/EM/pedigreeEM/pedigreeHyperGraph.h"
#include "/Users/Eddie/hyperGraphEM/src/tests/testMain.h"
#include "/Users/Eddie/hyperGraphEM/src/hyperGraph/hyperGraphAllocator.h"
#include "/Users/Eddie/hyperGraphEM/src/hyperGraph/nodeIterator.h"


void NodeIterator::NodeIteratorTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::operatorEqTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::getCurrentTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::getFamilyAddedFromTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::nextTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::getQueuedFamiliesTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::toSetTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void NodeIterator::getHashTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void LoopHandler::LoopHandlerTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}

void LoopHandler::getDependentNodesTests() {
    failWithMessage(__FILE__,__LINE__,"assert statement");
}
